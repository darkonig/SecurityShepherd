...
import servlets.admin.userManagement.DowngradeAdmin;
		...
		try
		{
			...
			else
			{
				...
				if(testuserId == null || testuserId.isEmpty()) {
					assert(Setter.userCreate(applicationRoot, null, testUsername, testUsername, "admin", testUsername+"@test.com", false));
					testuserId = Getter.getUserIdFromName(applicationRoot, testUsername);
				} else {
					Setter.updateUserRole(applicationRoot, testuserId, "admin"); // make sure user is admin, so downgrading makes sense
				}
