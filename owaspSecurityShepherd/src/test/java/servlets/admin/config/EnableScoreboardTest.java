...
public class EnableScoreboardTest 
{
	private static final org.apache.log4j.Logger log = Logger.getLogger(EnableScoreboardTest.class);
	...
	public String enableScoreboardDoPost(String classId, String csrfToken, String restricted) throws Exception
	{
		...
		try
		{
			...
			if(response.getStatus() != expectedResponseCode)
				fail("EnableScoreboard Servlet Returned " + response.getStatus() + " Code. " + expectedResponseCode + " Expected");
			else
			{
	...
	/**
	 * This test checks that non admin users get access errors when enabling the scoreboard
	 */
	@Test
	...
	/**
	 * This test checks that admin users can completely open the scoreboard (Null classID)
	 */
	@Test
			...
			else
			{
				...
				if(responseBody.contains(expectedResult))
				{
					...
					if(scoreboardClass.equalsIgnoreCase(classId))
					{
						...
						String message = "Did not update Scoreabord Setting to list specific class. Expected '" + classId + "' but got '" + scoreboardClass + "'";
						log.fatal(message);
						fail(message);
					}
				} 
