...
public class Hash
{
	...
	/**
	 * Generates HMAC with servers random encryption key on user name concatenated with level's base result key in a user friendly HTML form
		...
		if(userSpecificSolution != null)
		{
			toReturn = "<script>prepTooltips();prepClipboardEvents();</script>"
					+ "<div class='input-group'>" +
							"<textarea id='theKey' rows=2 style='height: 30px; display: inline-block; float: left; padding-right: 1em; overflow: hidden; width:85%'>"+userSpecificSolution+"</textarea>" +
	...
	/**
	 * Generates HMAC with servers random encryption key on user name concatenated with level's base result key
	 * @param baseKey The stored result key for the module
	 * @param userSalt Something specific to the user (User name)
